...
class TestNoLegacyCSSLinks(unittest.TestCase):
    ...
    def test_homepage_hides_platform_footer_shell(self):
        """
        La homepage Mission ne doit pas afficher le footer plateforme standard
        en plus du footer Mission (sinon doublon visuel).
        """
        index_tpl = os.path.join(THEME_TEMPLATES, "index.html")
        if not os.path.isfile(index_tpl):
            self.skipTest(f"Template homepage introuvable: {index_tpl}")

        with open(index_tpl, "r", encoding="utf-8", errors="ignore") as f:
            content = f.read()

        self.assertIn(
            "view-homepage-mf",
            content,
            "La homepage doit définir une classe body dédiée (view-homepage-mf).",
        )
        self.assertRegex(
            content,
            r"view-homepage-mf[\s\S]*mf-footer-shell[\s\S]*display\s*:\s*none",
            "La homepage doit masquer le footer plateforme (.mf-footer-shell) "
            "pour éviter le doublon de footer.",
        )
